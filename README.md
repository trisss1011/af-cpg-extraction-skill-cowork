...
- **v2.8.1** — (버그 수정) 6C 채팅 출력 구조에 **RoB 근거(AJ~AT 11열) 블록 누락 교정** — RoB가 엑셀에만 기록되고 채팅에 출력되지 않던 6B↔6C 불일치 해소(`⑤ RoB 근거` 블록 신설, 기존 불확실·NR→⑥). 매뉴얼 docx 동기화. 추출 규칙·열 구조·코드값 변경 없음(48열, 회의 결정 무관). (현재 버전)
