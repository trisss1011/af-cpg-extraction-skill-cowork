...
- **v2.9.1** — (2026-06-15 회의) **배제(exclude=Y) 논문 시트 처리 버그 패치** — 배제 논문은 기본정보 행만 보존하고 아웃컴·한의중재_한약 시트 내용은 자동 삭제(추출 미보존 + merge 제거 + 기존 마스터 소급). 동반 **merge-skill v2.9.1·upgrade-skill v2.9.1**(`v2.9.1_changes.md`, LATEST 2.9.1). + **comorbidity 코드 6(고혈전 색전) 과거 평가지표 인정**(과거 CHADS2 등도 발표 당시 기준 점수 충족 시 코드 6 부여) + **중복 논문 2케이스 처리**(완전 동일 삭제 / 번호만 다르면 후순위 exclude=Y, 카운트는 study_id 기준). + **버전 표기 통일(lockstep)**: 세 스킬·sample 모두 `2.9.1`로 정렬(작업자 현재 버전 명확화), `sample_v2.8.xlsx`→`sample_v2.9.1.xlsx` 리네임(내용 불변). 열 구조 v2.6 이후 동일(48열). (현재 버전)
