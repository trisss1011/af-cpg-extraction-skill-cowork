...
description: >
  ...
  v2.9.1 (2026-06-15, 구 2.7.1 — 배포 세트 버전 통일): **배제(exclude=Y) 논문 시트 처리** (Decision Log §9.7) — 추출 파일의 기본정보 `exclude=Y` 논문은 기본정보 행만 머지하고, 그 논문의 아웃컴·한의중재_한약 행은 머지에서 제외한다(배제 논문은 기본정보만 보존). cpg-data-extraction v2.9.1과 페어링(버전 번호를 배포 세트에 맞춰 통일).
...
10. **배제(exclude=Y) 논문 시트 제외** (v2.9.1): 추출 파일 `기본정보` 시트의 J열 `exclude=Y`인 논문은 **기본정보 행만 삽입**하고, 같은 번호의 아웃컴·한의중재_한약 행은 삽입하지 않는다. 배제 논문은 기본정보만 보존한다(Decision Log §9.7). 기본정보 행의 연분홍 서식은 추출 단계에서 부여되며 머지는 그대로 보존한다.
...
## 5단계: 중복 study_id 검증 + 번호순 a/b 재부여 (v2.7)

각 추출 파일의 `기본정보` 시트에서 study_id와 번호(A열)를 읽는다. study_id 중복은 **두 경우**로 구분한다 — 식별 키는 **번호(A열)**, 번호는 선정 관리 파일의 논문 고유번호다.

| 경우 | 판정 | 처리 |
|------|------|------|
| 같은 study_id **+ 같은 번호** | 동일 논문 재추출 | 충돌 → 기본 거부, 명시적 재머지 시만 overwrite (아래) |
| 같은 base study_id **+ 다른 번호** | 서로 다른 논문이 같은 저자_연도 | 거부하지 않고 **번호순 a/b/c 재부여** (5B) |

> **base study_id**: 끝에 단일 소문자 접미사가 붙어 있으면(`^(.+_\d{4})[a-z]$`) 그 접미사를 떼어낸 값. 예: `Zhang_2019a` → base `Zhang_2019`. 접미사가 없으면 study_id 자체가 base.

### 5A. 명시적 재머지 (overwrite)
...
### 5B. study_id 번호순 a/b 재부여

서로 다른 논문이 같은 base study_id를 갖는 경우를 번호 오름차순으로 분리한다. 머지에 포함될 **마스터 기존 행 + 신규 추출**을 모두 합쳐 base별로 재계산한다.

**알고리즘**:
1. (대상 수집) 마스터 `기본정보` + 이번에 머지될(거부되지 않은) 추출의 `(번호, study_id)`를 모은다.
2. (base 산출) 각 study_id에서 접미사를 떼어 base를 구한다 (`^(.+_\d{4})[a-z]$` → group1, 없으면 그대로).
3. (그룹핑) base별로 묶고, 각 base에 속한 **서로 다른 번호**를 오름차순 정렬한다.
4. (부여)
   - base에 번호가 **1개뿐**이면 접미사 없이 base 그대로 (`Zhang_2019`).
   - 번호가 **2개 이상**이면 번호 오름차순으로 `a`, `b`, `c` … 부여 (`Zhang_2019a`=최소 번호, …).
5. (적용) 산출된 study_id를 **세 시트 모두**에서 해당 번호의 모든 행에 일괄 기록한다. 기존 마스터 행도 접미사가 바뀌면 갱신한다.
6. (멱등) 이미 번호순으로 올바른 접미사면 변경 0건.

**미리보기 표시 (6단계 연동)**: 재부여가 발생하면 `study_id 재부여: Zhang_2019(50)→Zhang_2019b, Zhang_2019(30)→Zhang_2019a (base Zhang_2019: 2편)` 형태로 변경 내역을 모두 보여주고 승인받는다.

**주의**:
- 식별은 항상 **번호**로 한다. 같은 base+같은 번호는 동일 논문(5A overwrite 대상)이며 a/b 재부여 대상이 아니다.
- 재부여로 기존 마스터 study_id가 바뀔 수 있다(원칙 9). 변경 내역은 미리보기·머지 로그에 반드시 기록한다.
- 접미사는 study_id 셀에만 적용하고 번호(A열)는 건드리지 않는다.

> **3인 최종 머지**: 각 작업자 파일이 이미 `Zhang_2019a` 등을 갖고 있어도, base로 환원 후 전체 번호순으로 재계산되므로 작업자 간 접미사 충돌이 자동 해소된다.
...
for extract in sorted_extracts:
    n = extract.번호
    # v2.9.1: 배제 논문은 기본정보만 삽입 (아웃컴·한의중재_한약 시트 제외) — Decision Log §9.7
...
**배제 논문 처리 (v2.9.1)**: 추출 파일 기본정보 J열 `exclude=Y`이면 그 논문은 `기본정보` 시트에만 1행 삽입하고 `아웃컴`·`한의중재_한약` 시트에는 삽입하지 않는다(Decision Log §9.7). 따라서 행 수 검증의 expected에도 배제 논문의 아웃컴·한약 행 수는 더하지 않는다. 추출 파일 자체는 그대로 `merged/`로 이관(아웃컴·한약 데이터 원본 보존).
...
| 중복 study_id, **같은 번호** (명시 없음) | 동일 논문 → 해당 파일만 거부 |
| 중복 study_id, **같은 번호** (overwrite 명시) | 마스터에서 해당 study_id 전 시트 행 삭제 후 신규 삽입 |
| 같은 base study_id, **다른 번호** | 서로 다른 논문 → 거부 안 함, 번호순 a/b 재부여 (5B) |
| 백업 실패 | 머지 중단, 사용자 보고 |
...
### v2.9.1 (2026-06-15) — 구 2.7.1에서 배포 세트 버전으로 번호 통일
...
- 추출 파일 원본은 그대로 `merged/`로 이관(아웃컴·한약 데이터 보존). 기존 마스터의 배제 논문 두 시트 소급 삭제는 upgrade-skill v2.9.1(`v2.9.1_changes.md`) 담당.
- **버전 번호 통일**: 기능 변경분은 위 1건(배제 시트 제외)이며, 버전 번호를 `2.7.1` → `2.9.1`로 올린 것은 cpg-data-extraction·upgrade-skill과 배포 세트 버전을 맞추기 위함(lockstep). 이전 독립 이력은 아래 v2.7~v1.0 항목에 보존.

### v2.7 (2026-06-01) — 독립 버전 이력 (버전 통일 이전)
...
- 5단계를 "중복 두 경우 구분"으로 개정 — 같은 study_id+같은 번호는 동일 논문(overwrite), 같은 base+다른 번호는 a/b 재부여.
- 5B 재부여 알고리즘 추가(base 환원 → 번호순 그룹 → 접미사 부여 → 세 시트 일괄 적용 → 멱등). 미리보기·로그에 재부여 내역 표시.
- 3인 파일 최종 머지에서 작업자 간 접미사 충돌을 base 환원 후 번호순 재계산으로 자동 해소.
- 핵심 원칙 9 추가. 접미사 변경(churn)은 추출 단계 + 최종 재통일 전제하에 허용.
